...
enum DesignTokens {
    enum Colors {
        ...
        static func elevatedSurface(for colorScheme: ColorScheme) -> Color {
            surfaceElevated(for: colorScheme)
        }
    ...
    enum Layout {
        /// Maximum content width so large iPads maintain readable line lengths.
...
extension View {
    func onboardingCardShadow(colorScheme: ColorScheme) -> some View {
        shadow(color: colorScheme == .dark ? Color.black.opacity(0.6) : Color.black.opacity(0.14),
               radius: 12,
               x: 0,
               y: 6)
    }
